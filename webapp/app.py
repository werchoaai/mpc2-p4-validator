...
# Add webapp/ to path so outputs_p4 can be imported
sys.path.insert(0, str(Path(__file__).resolve().parent))
from outputs_p4 import (  # noqa: E402
    build_overview_workbook, build_detail_workbook, build_raw_workbook, _summary_row,
)
...
st.set_page_config(
    page_title="MPC² · P4 Legacy Validator — DL-EPR",
    page_icon="⚡",
    layout="wide",
...
def _check_password() -> bool:
    ...
    <div style="max-width:460px; margin:80px auto; padding:36px 40px;
      ...
      <h1 style="font-size:26px; font-weight:700; color:#4C4C4C; margin:0 0 8px;
        ...
        Projekt 4 · DL-EPR Legacy Validator
      </h1>
...
<div class="mpc-nav">
  <div class="mpc-nav-brand">
    ...
    <div>
      <div class="mpc-nav-title">Projekt 4 · DL-EPR Legacy Validator</div>
      <div class="mpc-nav-sub">ASC + HAD hochladen → Kennwerte + Diagramme + Integritäts-Check → Legacy-Excel</div>
    </div>
  </div>
...
with tab_analyse:
    ...
    with col_title:
        st.markdown("# Projekt 4 · DL-EPR Legacy Validator")
    with col_reset:
    ...
    if not uploaded:
        ...
        # Build P4-format rows once for all three downloads
        _p4_defaults = {"cell": "FC", "experiment_no": "", "order_no": order_no or "",
                       "limit": "", "solution_label": "146/120/1", "operator": "",
                       "h2so4": 146, "hcl": 120, "kscn": 1, "scan_rate": 1.67, "vertex_mv": 390}
        _rows_for_p4 = [_summary_row(m, _p4_defaults, i) for i, m in enumerate(measurements_for_append, 1)]

        dl_col1, dl_col2, dl_col3 = st.columns(3)

        # A) Overview workbook (legacy P4 format)
        with dl_col1:
            st.markdown("### 1) Overview (Legacy)")
            st.markdown(
                "<p style='font-size:13px; color:#666;'>Eine einzige Tabelle mit "
                "allen Messungen als Zeilen — Overview-Schema aus ON2024-0013 "
                "(Messung · Datum · Material · Ja/Jr/Qa/Qr · Verhältnisse).</p>",
                unsafe_allow_html=True,
            )
            try:
                ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                data = build_overview_workbook(_rows_for_p4)
                st.download_button(
                    label="Overview.xlsx herunterladen",
                    data=data,
                    file_name=f"Projekt4_Overview_{ts}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    width='stretch',
                )
            except Exception as e:
                st.error(f"Overview konnte nicht erstellt werden: {e}")

        # B) Detail workbook
        with dl_col2:
            st.markdown("### 2) Details (Legacy)")
            st.markdown(
                "<p style='font-size:13px; color:#666;'>Summary-Sheet + pro Messung "
                "ein Detail-Sheet mit Zeit · Potenzial · Strom · Stromdichte.</p>",
                unsafe_allow_html=True,
            )
            try:
                ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                data = build_detail_workbook(measurements_for_append, _rows_for_p4)
                st.download_button(
                    label="Details.xlsx herunterladen",
                    data=data,
                    file_name=f"Projekt4_Details_{ts}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    width='stretch',
                )
            except Exception as e:
                st.error(f"Details konnten nicht erstellt werden: {e}")

        # C) Raw workbook with Vorlage
        with dl_col3:
            st.markdown("### 3) Raw Workbook")
            st.markdown(
                "<p style='font-size:13px; color:#666;'>Legacy Raw-Format: "
                "<code>Vorlage</code>-Sheet + ein Roh-Sheet pro Messung.</p>",
                unsafe_allow_html=True,
            )
            try:
                ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                data = build_raw_workbook(measurements_for_append)
                st.download_button(
                    label="Raw.xlsx herunterladen",
                    data=data,
                    file_name=f"Projekt4_Raw_{ts}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    width='stretch',
                )
            except Exception as e:
                st.error(f"Raw workbook konnte nicht erstellt werden: {e}")
...
<div class="mpc-footer">
  ...
  <span style='font-size:12px; opacity:0.85;'>
    P4 Legacy Validator · gebaut von <a href="https://werchota.ai" target="_blank">werchota.ai</a> ·
